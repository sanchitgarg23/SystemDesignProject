...
/**
 * BookingService — Passenger Booking Operations
 * 
 * Handles the complete booking lifecycle: creation with 12:01 AM constraint,
 * fare calculation, QR code generation, retrieval, verification, and cancellation.
 * 
 * Design Pattern: Facade
 * - Hides the complexity of date validation, fare calculation, QR generation,
 *   and stop validation behind simple methods like createBooking().
 * 
 * OOP Concept: Encapsulation
 * - Business rules (booking window, fare formula, stop ordering) are
 *   encapsulated within the class, not scattered across controllers.
 */
export class BookingService {
  /**
   * Create a new booking with 12:01 AM constraint
   */
  public static async createBooking(userId: string, data: BookingInput) {
    const now = new Date();
    ...
    // Generate QR code (uses QRService)
    const { qrData, qrImage } = await QRService.generateBookingQR(bookingId);
   ...
   * Get booking by ID
   */
  public static async getBookingById(bookingId: string, userId?: string) {
   ...
   */
  public static async verifyBookingFromQR(qrData: string) {
    const verification = QRService.verifyBookingQR(qrData);
...
export const verifyBookingFromQR = BookingService.verifyBookingFromQR;
